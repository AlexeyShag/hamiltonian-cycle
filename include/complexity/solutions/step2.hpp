...
namespace complexity::solution2
{
    ...
    void operator()(auto& _batch) {
        ...
        for (auto& gen: _batch) {
            ...
            for (size_t i = 1; i < gen_size; ++i) {
                ...
                set.erase(id);
                gen.cost() += graph_.contains(current_id, id) * (multiplier--);
                current_id = id;
            }
            gen.cost() += graph_.contains(current_id, start_id)  * (multiplier--);
        }
